...
public final class Tester
{
	 ...
	 * @param testSuite 单元测试套件的class
	 */
	public void testAction(final Class<? extends Action> actionClass) throws Exception
	{
		ActionConfig ac = actionClass.getAnnotation(ActionConfig.class);
		Version version = (Version) ac.version().newInstance();
		Action act = FrameworkContext.CTX.getAction(version.getVersion(), actionClass.getSimpleName());
		if (act != null)
		{
			// 定义会话
			Session session = null;
			...
			{
				// 创建Session会话
				session = new SessionTesterImpl(act, httpSession, parameter);
				if (userId != 0)
				{
					session.setLoginInfo(userId);
				}
