			...
			if (encryptKey != null)
			{
				...
				if (sbf != null && !sbf.isEmpty() && seed != null && !seed.isEmpty())
				{
					...
					if (entryText.equals(sbf))
					{
						...
						session.denied("access denied for enctrypt sid denctrypt error");
						return;
					}
					session.denied("access denied for enctrypt error");
					return;
				}
				session.denied("access denied for sbf or seed null");
				return;
			}
			session.denied("access denied for encryptKey null");
			return;
		}
		session.unlogin();
	}
