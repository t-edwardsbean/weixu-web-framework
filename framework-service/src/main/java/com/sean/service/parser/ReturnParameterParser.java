...
import com.sean.service.writer.FieldWriterSimple;
...
public class ReturnParameterParser
{
	public ReturnParameterEntity parse(Object obj, Field field) throws Exception
	{
		...
		String[] jsonFields = new String[0];
		...
		{
		case Numeric:
			fieldWriter = FieldWriterSimple.getInstance();
			break;
		case String:
			fieldWriter = FieldWriterSimple.getInstance();
			break;
		...
		ReturnParameterEntity pe = new ReturnParameterEntity(field.get(obj).toString(), rpc.format(), rpc.entity(), fields, udes, jsonFields,
				rpc.descr(), fieldWriter);
		return pe;
	}
